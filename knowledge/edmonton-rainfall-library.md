...
| Alberta 511 highway cameras | **Merged** (map layer, default off) | 17 cams around Edmonton (Henday ring road + approaches). The list API (`511.alberta.ca/api/v2/get/cameras`) blocks browser CORS → camera **locations are a baked snapshot** (2026-07-12) in `dashboard/index.html` (`CAMERAS` const); **images** (`/map/Cctv/<id>`) are open-CORS PNGs fetched live with a cache-buster. Re-run the API curl to refresh the location list if cams are added/moved |
